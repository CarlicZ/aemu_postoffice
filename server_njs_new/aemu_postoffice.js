...
var PtpState;
(function (PtpState) {
    PtpState[PtpState["PTP_STATE_WAITING_FOR_LISTEN"] = -2] = "PTP_STATE_WAITING_FOR_LISTEN";
...
function remove_existing_and_insert_session(ctx, name) {
    ...
    if (existing_session != undefined) {
        ...
        if (existing_session.state === SessionMode.SESSION_MODE_PTP_LISTEN) {
            const has_waiting_connect = Object.values(sessions).some(s => s.state === SessionMode.SESSION_MODE_PTP_CONNECT &&
...
function try_pair_ptp_connect(ctx) {
...
function notify_waiting_connects(listen_session) {
    const waiting = Object.values(sessions).filter(s => s.state === SessionMode.SESSION_MODE_PTP_CONNECT &&
        s.ptp_state === PtpState.PTP_STATE_WAITING_FOR_LISTEN &&
