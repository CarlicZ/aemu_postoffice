...
function remove_existing_and_insert_session(ctx, name){
	...
	if (existing_session != undefined){
		...
		if (existing_session.state === "ptp_listen"){
			const has_waiting_connect = Object.values(sessions).some(s =>
				s.state === "ptp_connect" &&
				s.ptp_state === "waiting_for_listen" &&
	...
	switch(type){
		...
		case AEMU_POSTOFFICE_INIT_PTP_CONNECT:{
			...
			if (try_pair_ptp_connect(ctx)){
				...
				ctx.ptp_state = "waiting_for_listen";
				...
				const try_connect_deferred = () => {
					...
					if (attempts < PTP_CONNECT_RETRY_MAX){
						...
						let stat_obj = get_statistics_obj(ctx.ip);
						stat_obj.ptp_deferred_failures++;
						close_session(ctx);
					}
				};
...
function data_debug(request, response){
	...
	let convert_session_list = (from_list) => {
		let to_list = {}
		for (const [key, sessions_group] of Object.entries(from_list)){
			let response_sessions = [];
			to_list[key] = response_sessions;
			for(const session of Object.values(sessions_group)){
